...
class ToolExecutor:
    ...
    def execute_tool(tool_name, args_str):
        ...
        if tool_name == "get_current_time":
            ...
            return ToolExecutor.python_exec(args_str)
             ...
             query = args_str.strip().strip('"').strip("'")
             return ToolExecutor.search_google(query)
        elif tool_name == "generate_image":
             # Extract prompt
             prompt = args_str.strip().strip('"').strip("'")
             return ToolExecutor.generate_image(prompt)
        return "Unknown tool"

    @staticmethod
    def search_google(query):
        # Use process_generation_core to perform a search via Gemini
        # We need to access process_generation_core which is defined in global scope
        # Since this runs at runtime, it should be available.
        try:
            print(f"[ToolExecutor] Synthesizing search for: {query}")
            success, result = process_generation_core(
                prompt_input=f"Search for this and provide a detailed summary: {query}", 
                is_stream=False, 
                model_name="gemini-3-flash", 
                use_search=True
            )
            if success:
                return f"Ensure you cite this data: {result}"
    ...
    def generate_image(prompt):
        try:
            print(f"[ToolExecutor] Generating image for: {prompt}")
            # Use process_generation_core with image model. 
            # Note: process_generation_core is defined globally, so we can use it.
            success, result = process_generation_core(
                prompt_input=prompt, 
                is_stream=False, 
            ...
            if success:
                 # Result is the raw Gemini JSON response specific to image gen
                 # We need to extract the base64 data.
                 # Structure usually: result['candidates'][0]['content']['parts'][0]['inlineData']['data']
                 # Or sometimes directly in response depending on upstream API version.
                 
                 # Check if result is a string (error message) or dict
                 if isinstance(result, str):
                     return f"Image Generation Error: {result}"
                 
                 try:
                     # Check for 'candidates' or 'responses'
                     candidates = result.get('candidates', [])
                     if not candidates and 'response' in result:
                     ...
                     return "Image generation successful but no image data found in response."
                 except Exception as e:
                     return f"Image parsing failed: {e}"
            
            return "Image generation failed."
        except Exception as e:
            return f"Image Generation Exception: {e}"
...
def run_smart_agent_logic(prompt, messages, model_name, allow_image_tool=True):
    # This encapsulates the original smart_agent logic so it can be called by threads or main
    
    # ReAct Loop capability check
    react_keywords = ["url", "link", "visit", "đọc báo", "web", "tính toán", "calculate", "code", "run", "thời gian", "time", "analysis", "execute", "tìm kiếm", "tra cứu", "search", "find", "dữ liệu", "số liệu", "data", "thông tin"]
    writing_keywords = ["viết bài", "write article", "viết blog", "blog", "nghiên cứu", "research", "tổng hợp", "analyze", "phân tích", "soạn thảo", "lên kế hoạch", "plan", "dàn ý", "outline", "bài viết", "content", "nội dung", "sáng tạo", "vẽ", "minh họa"]
    
    enable_react = any(k in prompt.lower() for k in react_keywords + writing_keywords)
    is_writing_task = any(k in prompt.lower() for k in writing_keywords)

    # Simple Heuristics for initial model selection
    image_keywords = ["generate image", "tạo ảnh", "create a picture", "make an image"]
    
    # Check "vẽ" but only if not accompanied by writing tasks detailed above is tricky, 
    # but "enable_react" covers writing.
    # We only default to pure image model if it's NOT a complex task
    if any(k in prompt.lower() for k in image_keywords) and not enable_react:
        model_name = "gemini-3-pro-image"
    elif "vẽ" in prompt.lower() and not is_writing_task and len(prompt.split()) < 10:
        model_name = "gemini-3-pro-image"
    ...
    final_response_text = ""
    thought_log = [] # To store the thinking process
    image_artifacts = {} # To store heavy image data
    image_counter = 1
    ...
    if enable_react and "gemini-3-pro-image" not in model_name:
        ...
        max_turns = 7 # Increased turns for research
...
4. search_google(query): Searches Google and returns a summary.
"""
        if allow_image_tool:
             tools_desc += "5. generate_image(prompt): Generates an image based on the prompt and returns an image tag.\n"
        
        system_instr = f"""
You are a smart AI assistant capable of deep reasoning and data synthesis.
You have access to the following tools:
{tools_desc}

PROCESS Requirements:
1. **PLAN**: First, think and plan your actions. Output "Thinking: [Plan]".
2. **EXECUTE**: Use tools `generate_image` or `search_google` to gather assets. Do this BEFORE writing the full article.
3. **SYNTHESIZE**: Once you have the image tag or search data, write the final complete article.
"""
        if allow_image_tool:
             system_instr += "\nIMPORTANT: If the user asks for an image, you MUST call `generate_image` immediately."
...
"""
        ...
        for turn in range(max_turns):
            ...
            thought_match = re.search(r'(Thinking:.*?)(?:<tool_code>|$)', result_text, re.DOTALL | re.IGNORECASE)
            if thought_match:
                thought_content = thought_match.group(1).strip()
                thought_log.append(thought_content)
            elif tool_match:
                 # If tool is used but no explicit "Thinking:" tag, treat the text before tool as thought
                 pre_tool_text = result_text.split("<tool_code>")[0].strip()
                 if pre_tool_text:
                     thought_log.append(pre_tool_text)
            ...
            if tool_match:
                ...
                # OPTIMIZATION: Handle heavy image data
                if func_name == "generate_image" and "data:image" in tool_output:
                     # Extract the full tag
                     full_tag = tool_output
                     placeholder = f"![Image {image_counter}]([IMAGE_{image_counter}])"
                     image_key = f"[IMAGE_{image_counter}]"
                     
                     placeholder_simple = f"[IMAGE_{image_counter}]"
                     image_artifacts[placeholder_simple] = full_tag
                     
                     # Replace output for history
                     tool_output_for_llm = f"Image generated successfully. You MUST include it in your article using exactly this placeholder: {placeholder_simple}"
                     image_counter += 1
                else:
                     tool_output_for_llm = tool_output

                # Append to history - ensure role is model for assistant messages
                react_history.append({"role": "assistant", "content": result_text})
                react_history.append({"role": "user", "content": f"TOOL OUTPUT: {tool_output_for_llm}\n\nContinue providing the answer."})
                
                thought_log.append(f"> **Action**: Executed `{func_name}`.")
                print(f"[SmartAgent] Turn {turn} Tool {func_name} output len: {len(tool_output)}") # Still print full len for debug
            else:
        ...
        # Reinject Image Artifacts
        if image_artifacts:
             for placeholder, full_tag in image_artifacts.items():
                if placeholder in final_response_text:
                    final_response_text = final_response_text.replace(placeholder, full_tag)
                else:
                    final_response_text += f"\n\n{full_tag}"
                    
        # Return tuple
        return True, final_response_text, thought_log, use_search

    else:
        # Standard Path
        # Online / Search detection
        search_keywords = ["search", "tìm kiếm", "online", "current", "latest", "mới nhất", "hôm nay"]
        use_search = any(k in prompt.lower() for k in search_keywords) or "-online" in messages[-1].get('model', '') if messages else False
        ...
        if success:
             return True, final_response_text, [], use_search
        else:
             return False, final_response_text, [], False

@app.route('/v1/agent/smart', methods=['POST'])
def smart_agent_route(return_openai_format=False):
    # Route Handler
    body = request.json
    
    # Delegate to logic handler
    result = smart_agent_logic(body)
    
    if isinstance(result, tuple):
         success, text, thoughts, search_used = result
         if not success: 
             return jsonify({"error": text}), 500
         
         # Add Thinking Log if present
         if thoughts:
             formatted_thoughts = "\n\n".join([f"> {t}" for t in thoughts])
             # Check if mode was parallel or serial, but thoughts usually go on top
             # If parallel, text already has thoughts formatted? No, logic returns raw thoughts
             # Let's verify smart_agent_logic for parallel...
             # Ah, in parallel mode, it returns a jsonify object!
             pass
         
         # Logic returned tuple (Serial mode or Serial ReAct)
         final_text = text
         if thoughts:
              formatted_thoughts = "\n\n".join([f"> {t}" for t in thoughts])
              if "### Process of Thinking" not in final_text:
                  final_text = f"### Process of Thinking & Synthesis\n{formatted_thoughts}\n\n---\n\n{final_text}"
        
         return jsonify({"result": final_text, "model": "gemini-3-flash", "search_used": search_used})

    # If result is already a Response object (from parallel mode which returns jsonify)
    return result

def smart_agent_logic(body):
    ...
    if not prompt and not messages:
        return False, "No prompt or messages provided", [], False

    # 1. Routing & Setup
    model_name = "gemini-3-flash"
    
    # Analyze Intent for Parallel execution
    # Keywords indicating BOTH writing AND drawing
    image_keywords = ["vẽ", "tạo ảnh", "create a picture", "make an image", "generate image", "minh họa", "draw"]
    writing_keywords = ["viết", "write", "viết bài", "write article", "viết blog", "blog", "nghiên cứu", "research", "tổng hợp", "soạn thảo", "lên kế hoạch", "dàn ý", "outline", "bài viết", "content", "nội dung", "thơ", "poem", "truyện", "story"]
    
    needs_image = any(k in prompt.lower() for k in image_keywords)
    needs_writing = any(k in prompt.lower() for k in writing_keywords)
    
    if needs_image and needs_writing and "gemini-3-pro-image" not in body.get('model', ''):
        print(f"[SmartAgent] PARALLEL MODE DETECTED. Executing Writing and Drawing concurrently.")
        
        # Define output containers
        results = {"text": None, "image": None, "thought_log": []}
        
        def run_writer():
            # Modify prompt to ask for a placeholder
            writer_prompt = prompt + "\n\nIMPORTANT: For the image/illustration requested, DO NOT call the image tool. Instead, place the tag [PENDING_IMAGE_1] exactly where the image should go in the article. Focus on writing the content."
            
            # Call core logic (we need to refactor smart_agent logic or call a helper)
            # We call run_smart_agent_logic helper
            success, text, thoughts, _ = run_smart_agent_logic(writer_prompt, messages, model_name, allow_image_tool=False)
            if success:
                results["text"] = text
                results["thought_log"] = thoughts
            else:
                results["text"] = f"Writing failed: {text}"

        def run_painter():
            # Extract just the image prompt part (naive approach for now, or just send full prompt to image model)
            print(f"[SmartAgent] Parallel Painter starting with prompt: {prompt[:50]}...")
            tool_res = ToolExecutor.generate_image(prompt)
            if "data:image" in tool_res:
                results["image"] = tool_res # This is the full markdown tag
            else:
                 results["image"] = None # Failed or text error
        
        # Spawn Threads
        t_writer = threading.Thread(target=run_writer)
        t_painter = threading.Thread(target=run_painter)
        ...
        if results["image"]:
            if "[PENDING_IMAGE_1]" in final_text:
                final_text = final_text.replace("[PENDING_IMAGE_1]", results["image"])
            else:
                # Append if placeholder missing
                final_text += f"\n\n{results['image']}"
        
        # Format Thoughts
        thought_log = results["thought_log"]
        if thought_log:
             formatted_thoughts = "\n\n".join([f"> {t}" for t in thought_log])
             final_text = f"### Process of Thinking (Parallel)\n{formatted_thoughts}\n\n---\n\n{final_text}"
             
        # Return success tuple for parallel, OR jsonify directly? 
        # The caller expects tuple or Response.
        # Let's return a Response object to bypass caller logic
        return jsonify({"result": final_text, "model": model_name, "search_used": False, "mode": "parallel"})

    # ... Existing logic continues below (refactored to ensure fallback)
    return run_smart_agent_logic(prompt, messages, model_name)
...
def process_generation_core(prompt_input, is_stream, model_name="gemini-3-flash", use_search=False):
    ...
    for account_id in account_ids:
        ...
        if is_image_gen and any(x in project_id for x in ["useful-", "bright-", "swift-", "calm-", "bold-"]):
             print(f"Skipping account {email} for Image Gen due to Mock PID {project_id}")
             continue
